...
best_rmse <- rf_grid_result %>% 
  select_by_pct_loss("trees",
                     metric = "rmse",
                     limit = 2  #here we accepted 2% loss
  )
...
best_r2 <- rf_grid_result %>%
  #select_best(metric = 'rsq")     #here we can select best model
  select_by_pct_loss("trees",
                     metric = "rsq",
                     limit = 2
                     )  #here we accepted 2% loss
...
```{r final_fit}
#APPLYING TRAINING TO ENTIRE TRAINING SET AND USING IT IN TETS SET
...
Test set has higher RMSE (2.94 vs 1.21) and lower R2 (0.95 vs 0.18) in comparison to train set
